...
class CCebNode : public CCebElement 
	// 연결된 큐를 위한 노드 클래스
	// 연결된 큐에 필요한 데이터 값이 들어있는 클래스 CCebElement 클래스를 상속
{
public:
...
public:
	...
	char GetChar(){ // 문자형 변수를 반환하는 함수 
		return getChar(); // CCebElement의 getChar 함수를 호출하여 문자형 변수를 반환할 수 있도록 한다
	}

	CCebNode* RemoveNext( ) {
		// 연결된 큐에 다음 값을 제거하는 함수
		// 임의의 연결된 큐의 링크 값을 선언해 현재 노드의 링크값(다음 노드의 주소가 들어있는)을 대입해주고
